...
| `scope` |  String<br/>Required | One or more  available scopes.  <ul><li>[Front-oice scopes](https://auth..net/.well-known/openid-configuration).</li><li>[Back-oice scopes](https://authbo..net/.well-known/openid-configuration)</li></ul>Optionally, add the following special scopes:<ul><li>`openid`: if the `response_type` includes `id_token`, you **must** add this scope ro receive the identity scope.</li><li>`oline_access`: add this scope to request a `refresh_token`. </li></ul> |
...
```csharp
var ru = new RequestUrl("https://auth..net/connect/authorize");
